...
Singleton {
    ...
    Timer {
        ...
        onTriggered: {
            if (refreshToplevelsPending) {
                Hyprland.refreshToplevels();
                refreshToplevelsPending = false;
            }
            if (refreshWorkspacesPending) {
                Hyprland.refreshWorkspaces();
                refreshWorkspacesPending = false;
            }
            if (refreshMonitorsPending) {
                Hyprland.refreshMonitors();
                refreshMonitorsPending = false;
            }
        }
    }
    ...
    Connections {
        function onRawEvent(event: HyprlandEvent): void {
            ...
            if (n === "configreloaded") {
                ...
                refreshTimer.refreshWorkspacesPending = true;
                refreshTimer.refreshMonitorsPending = true;
                refreshTimer.restart();
            } else if (["openwindow", "closewindow", "movewindow"].includes(n)) {
                refreshTimer.refreshToplevelsPending = true;
                refreshTimer.refreshWorkspacesPending = true;
                refreshTimer.restart();
