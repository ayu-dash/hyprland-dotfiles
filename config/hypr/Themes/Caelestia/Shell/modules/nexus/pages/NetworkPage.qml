...
PageBase {
    ...
    signal networkSelected(ap: Iwd.AccessPoint)
    ...
    ColumnLayout {
        ...
        Timer {
            running: root.visible && Iwd.wifiEnabled
            repeat: true
            triggeredOnStart: true
            interval: GlobalConfig.nexus.networkRescanInterval
            onTriggered: Iwd.rescanWifi()
        }
        ...
        ToggleRow {
            ...
            checked: Iwd.wifiEnabled
            onToggled: Iwd.enableWifi(checked)
        }

        ItemList {
            id: networkList

            showList: Iwd.wifiEnabled
            placeholderIcon: Iwd.wifiEnabled ? "wifi_find" : "signal_wifi_off"
            placeholderText: Iwd.wifiEnabled ? qsTr("No networks found") : qsTr("Wi-Fi disabled")
            extraHeight: Iwd.scanning ? Tokens.rounding.extraSmall : 0 // Inline so it isn't affected by anim
            list.anchors.top: scanningIndicator.bottom

            model: ScriptModel {
                values: {
                    const connecting = Iwd.connectingSsid();
                    // Lower rank sorts higher in the list
                    const rank = n => n.active ? 0 : n.ssid === connecting ? 1 : Iwd.hasSavedProfile(n.ssid) ? 2 : 3;
                    return [...Iwd.networks].sort((a, b) => rank(a) - rank(b) || b.strength - a.strength);
                }
            }
            ...
            delegate: StateLayer {
                ...
                required property Iwd.AccessPoint modelData
                property bool currentSelected
                property real textOpacity: disabled ? 0.5 : 1

                disabled: currentSelected || Iwd.connectingSsid() === modelData.ssid
                ...
                Connections {
                    function onNetworkSelected(ap: Iwd.AccessPoint): void {
                ...
                RowLayout {
                    ...
                    ColumnLayout {
                        ...
                        StyledText {
                            Layout.fillWidth: true
                            text: qsTr("Security: %1%2").arg(network.modelData.security).arg(network.modelData.active ? qsTr(" • Connected") : Iwd.hasSavedProfile(network.modelData.ssid) ? qsTr(" • Saved") : "")
                    ...
                    AnimLoader {
                        sourceComp: Iwd.connectingSsid() === network.modelData.ssid ? loadingComp : iconComp
            ...
            StyledProgressBar {
                ...
                anchors.margins: 1
                implicitHeight: Iwd.scanning ? Tokens.rounding.extraSmall : 0
                indeterminate: true
