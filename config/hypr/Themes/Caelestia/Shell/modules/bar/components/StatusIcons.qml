...
StyledRect {
    ...
    ColumnLayout {
        ...
        WrappedLoader {
            name: "network"
            active: Config.bar.status.showNetwork && (!Iwd.activeEthernet || Config.bar.status.showWifi)

            sourceComponent: MaterialIcon {
                animate: true
                text: Iwd.active ? Icons.getNetworkIcon(Iwd.active.strength ?? 0) : "wifi_off"
                color: root.colour
            }
        }
        ...
        WrappedLoader {
            name: "ethernet"
            active: Config.bar.status.showNetwork && Iwd.activeEthernet
