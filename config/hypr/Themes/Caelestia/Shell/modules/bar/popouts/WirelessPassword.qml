...
ColumnLayout {
    ...
    function checkConnectionStatus(): void {
        ...
        const isConnected = root.network && Iwd.active && Iwd.active.ssid && Iwd.active.ssid.toLowerCase().trim() === root.network.ssid.toLowerCase().trim();
        ...
        if (Iwd.pendingConnection === null && connectButton.connecting) {
            ...
            if (connectionMonitor.repeatCount > 10) {
                ...
                if (root.network && root.network.ssid) {
                    Iwd.forgetNetwork(root.network.ssid);
                }
            }
        }
    }
    ...
    Timer {
        ...
        onTriggered: {
            ...
            if (root.shouldBeVisible && Iwd.active && Iwd.active.ssid) {
                const stillConnected = Iwd.active.ssid.toLowerCase().trim() === root.network.ssid.toLowerCase().trim();
                if (stillConnected) {
                    connectionMonitor.stop();
                    connectButton.connecting = false;
                    connectButton.text = qsTr("Connect");
