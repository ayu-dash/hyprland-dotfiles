...
ColumnLayout {
    ...
    Toggle {
        ...
        toggle.onToggled: Iwd.enableWifi(checked)
    }
    ...
    StyledText {
        ...
        Layout.rightMargin: Tokens.padding.extraSmall
        text: qsTr("%1 networks available").arg(Iwd.networks.length) // qmllint disable missing-property
        ...
        RowLayout {
            ...
            StyledRect {
                ...
                StateLayer {
                    ...
                    disabled: networkItem.loading || !Iwd.wifiEnabled

                    onClicked: {
                        if (networkItem.modelData.active) {
                            Iwd.disconnectFromNetwork();
                        } else {
    ...
    StyledRect {
        ...
        StateLayer {
            ...
            disabled: Iwd.scanning || !Iwd.wifiEnabled
            onClicked: Iwd.rescanWifi()
        }
    ...
    StyledText {
        ...
        Layout.topMargin: visible ? Tokens.spacing.small : 0
        Layout.rightMargin: Tokens.padding.extraSmall
        text: qsTr("%1 devices available").arg(Iwd.ethernetDevices.length)
        color: Colours.palette.m3onSurfaceVariant
        font: Tokens.font.body.small
    }
    ...
    Repeater {
        visible: root.view === "ethernet"
        model: ScriptModel {
            values: [...Iwd.ethernetDevices].sort((a, b) => {
                ...
                StateLayer {
                    ...
                    onClicked: {
                        if (ethernetItem.modelData.connected && ethernetItem.modelData.connection) {
                            Iwd.disconnectEthernet(ethernetItem.modelData.connection, () => {});
                        } else {
                            Iwd.connectEthernet(ethernetItem.modelData.connection || "", ethernetItem.modelData.interface || "", () => {});
                        }
                    }
                }
    ...
    Connections {
        function onActiveChanged(): void {
            if (Iwd.active && root.connectingToSsid === Iwd.active.ssid) {
                root.connectingToSsid = "";
                // Close password dialog if we successfully connected
                if (root.showPasswordDialog && root.passwordNetwork && Iwd.active.ssid === root.passwordNetwork.ssid) {
