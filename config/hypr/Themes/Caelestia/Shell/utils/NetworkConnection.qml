...
QtObject {
    ...
    function handleConnect(network, session, onPasswordNeeded): void {
        ...
        if (NetworkManager.active && NetworkManager.active.ssid !== network.ssid) {
    ...
    function connectToNetwork(network, session, onPasswordNeeded): void {
        ...
        if (network.isSecure) {
            const hasSavedProfile = NetworkManager.hasSavedProfile(network.ssid);

            if (hasSavedProfile) {
                NetworkManager.connectToNetwork(network.ssid, "", network.bssid, null);
            } else {
                // Use password check with callback
                NetworkManager.connectToNetworkWithPasswordCheck(network.ssid, network.isSecure, result => {
                    if (result.needsPassword) {
                        // Clear pending connection if exists
                        if (NetworkManager.pendingConnection) {
                            NetworkManager.connectionCheckTimer.stop();
                            NetworkManager.immediateCheckTimer.stop();
                            NetworkManager.immediateCheckTimer.checkCount = 0;
                            NetworkManager.pendingConnection = null;
                        }
    ...
    function connectWithPassword(network, password, onResult): void {
        ...
        NetworkManager.connectToNetwork(network.ssid, password || "", network.bssid || "", onResult || null);
    }
}
